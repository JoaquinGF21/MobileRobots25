...
controller.set_speed_r(0.75)
controller.set_speed_l(0.75)
move(controller=controller, Vl=5, Vr=5, D=1300, T=0.2)  # p0->p4

controller.set_speed_r(0.5)
controller.set_speed_l(0)
move(controller=controller, Vl=5, Vr=5, D=145, T=0.2)  # turn left

controller.set_speed_r(0.5)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=1150, T=0.2)  # p4->p8
...
controller.set_speed_r(0.5)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=220, T=0.2)  # p8->p9
...
controller.set_speed_r(0.5)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=720, T=0.2)  # p9->p11

controller.set_speed_r(0)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=145, T=0.2)  # turn right

controller.set_speed_r(0.5)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=230, T=0.2)  # p11->p12
...
controller.set_speed_r(0.5)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=720, T=0.2)  # p12->p14
...
controller.set_speed_r(0.5)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=500, T=0.2)  # p14->p15
...
controller.set_speed_r(0.5)
controller.set_speed_l(0.5)
move(controller=controller, Vl=5, Vr=5, D=800, T=0.2)  # p15->17


# A function to periodically print the motor speed values
def print_speeds():
    while True:
        print(f"Motor L Speed: {controller.get_speed_l()}, Motor R Speed: {controller.get_speed_r()}")
        time.sleep(1)  # print every second

# Start the print thread
print_thread = threading.Thread(target=print_speeds)
print_thread.daemon = True  # This ensures the thread will exit when the main program exits
print_thread.start()

# Map motor L and R in the range (-1,1) at 0.05 intervals
speeds = [i*0.05 for i in range(-20, 21)]

for S in speeds:
    controller.set_speed_l(S)
    time.sleep(5)
    for S in speeds:
        controller.set_speed_r(S)
        time.sleep(5)
